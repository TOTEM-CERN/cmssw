  ...
  originalLabel(pSet.getParameter<string>("originalLabel"))
{
  ...
  // initialize random engine   - commented cause rand is not needed
...
void SmearingGenerator::produce(edm::Event& event, const edm::EventSetup& es)
{
  ...
  unique_ptr<HepMCProduct> output(new HepMCProduct(*mcProd));

  event.put(move(output), originalLabel);
  ...
  ApplyBeamSmearing(evt, rndEng);
  ApplyVertexSmearing(evt, rndEng);
...
void SmearingGenerator::ApplyVertexSmearing(HepMC::GenEvent *evt, CLHEP::HepRandomEngine* engine)
{
  // generate vertex smearing
//  double v_x = rand->fire(MeanX, SiX);
//  double v_y = rand->fire(MeanY, SiY);
//  double v_z = rand->fire(MeanZ, SiZ);
  double v_x = CLHEP::RandGaussQ::shoot(engine, MeanX, SiX);
  double v_y = CLHEP::RandGaussQ::shoot(engine, MeanY, SiY);
  double v_z = CLHEP::RandGaussQ::shoot(engine, MeanZ, SiZ);
  double v_t = 0.;  
...
void SmearingGenerator::ApplyBeamSmearing(HepMC::GenEvent *evt, CLHEP::HepRandomEngine* engine)
{
